...
def teacache_forward(
    ...
    img_mod_params = self.transformer_blocks[0].img_mod(temb_)  # [B, 6*dim]
    txt_mod_params = self.transformer_blocks[0].txt_mod(temb_)  # [B, 6*dim]
    # Split modulation parameters for norm1 and norm2
    img_mod1, img_mod2 = img_mod_params.chunk(2, dim=-1)  # Each [B, 3*dim]
    txt_mod1, txt_mod2 = txt_mod_params.chunk(2, dim=-1)  # Each [B, 3*dim]

    img_normed = self.transformer_blocks[0].img_norm1(inp)
    img_modulated, img_gate1 = self.transformer_blocks[0]._modulate(img_normed, img_mod1)

    txt_normed = self.transformer_blocks[0].txt_norm1(inp1)
    txt_modulated, txt_gate1 = self.transformer_blocks[0]._modulate(txt_normed, txt_mod1)

    cache_key = tuple(encoder_hidden_states.shape)
    # self.modulated_inp.append(temb.clone().to('cpu'))
    if cache_key not in self.modulated_inp_dict:
        self.modulated_inp_dict[cache_key] = []
    self.modulated_inp_dict[cache_key].append(img_modulated.clone().to('cpu'))
    ...
    if cache_key not in self.pre_res_dict:
        self.pre_res_dict[cache_key] = []
    self.pre_res_dict[cache_key].append(previous_residual.clone().to('cpu'))
...
def worker_process(gpu_id, prompts, args, start_idx):
    ...
    for local_idx, prompt in enumerate(prompts):
        ...
        # Reset cache
        pipeline.transformer.__class__.modulated_inp_dict = {}
        pipeline.transformer.__class__.pre_res_dict = {}
        ...
        # Get diff data
        if len(pipeline.transformer.__class__.modulated_inp_dict) !=2:
            print("positive len equal to negative len, skip this prompt")
            continue
        else:
            for key in pipeline.transformer.__class__.modulated_inp_dict:
                inp_diff, pre_resdu_diff = get_data_diff(
                    pipeline.transformer.__class__.modulated_inp_dict[key], 
                    pipeline.transformer.__class__.pre_res_dict[key]
                )
                print(f"GPU {gpu_id}: Processed {len(inp_diff)} inp diffs and {len(pre_resdu_diff)} pre_res diffs for key {key}")
...
def fit_diff_data(all_inp_diff, all_pre_res_diff):
    ...
    # print(inp_diff_np.shape, pre_res_diff_np.shape)
    positive_inp_diff_np = inp_diff_np[::2, :]
    positive_pre_res_diff_np = pre_res_diff_np[::2, :]
    negative_inp_diff_np = inp_diff_np[1::2, :]
    negative_pre_res_diff_np = pre_res_diff_np[1::2, :]

    x = positive_inp_diff_np.mean(axis=0)
    y = positive_pre_res_diff_np.mean(axis=0)
    ...
    plt.tight_layout()
    plt.savefig('residual_polynomial_fitting_log_positive.png')
    print("Polynomial positive coefficients:", coefficients)

    x = negative_inp_diff_np.mean(axis=0)
    y = negative_pre_res_diff_np.mean(axis=0)
    coefficients = np.polyfit(x, y, 4)
    rescale_func = np.poly1d(coefficients)
    ypred = rescale_func(x)
    plt.clf()
    ...
    plt.title('4th order My Polynomial fitting ')
    plt.tight_layout()
    plt.savefig('residual_polynomial_fitting_log_negative.png')
    print("Polynomial negative coefficients:", coefficients)
