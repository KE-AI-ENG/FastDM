...
flash_mha(torch::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    TORCH_CHECK(q_dtype == torch::ScalarType::Float8_e4m3fn,
                "flash_mha only support fp8-e4m3 data type");
    ...
    TORCH_CHECK(num_heads == num_heads_k, "Number of heads in key/value must equal to query");
    ...
    torch::Tensor descale_q, descale_k, descale_v;
    if(q_dtype == torch::ScalarType::Float8_e4m3fn) {
        descale_q = torch::ones({1}, opts.dtype(torch::kFloat));
        descale_k = torch::ones({1}, opts.dtype(torch::kFloat));
        descale_v = torch::ones({1}, opts.dtype(torch::kFloat));
